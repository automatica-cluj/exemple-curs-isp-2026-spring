...
Arduino sketch for the ESP32-C3 Super Mini that reads onboard sensors and publishes data to an MQTT broker every 5 seconds. It also listens for LED control commands.
...
- **`callback()`** — Handles incoming MQTT messages; checks if the topic ends with `/led` and toggles GPIO 8 accordingly
