...
Also exposes a `publish()` method used by the controllers to send LED and OTA commands back to devices.
...
Stores both the device identity (MAC address, IP) and the latest sensor values (temperature, RSSI, heap, uptime, etc.) directly on the entity. Also tracks OTA-related state: `firmwareVersion`, `otaStatus`, and `otaLastAttempt`. This avoids joins when the dashboard fetches all devices with their current readings.
...
| `js/app.js`     | Polls `GET /api/devices` every 5s, renders device cards, LED commands, firmware upload, OTA deploy |
