...
async function sendPowerModeCommand(deviceId, mode) {
    const response = await fetch(API_BASE + '/devices/' + deviceId + '/power_mode', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mode: mode })
    });
    if (!response.ok) {
        throw new Error('Failed to send power mode command');
    }
}
...
function renderDeviceCard(device) {
    ...
    var html = ''
        ...
        + '      <span class="power-status ' + (device.powerMode === 'low' ? 'low' : 'normal') + '"></span>'
        + '      Power: ' + (device.powerMode || 'normal')
        + '      | Interval: ' + (device.telemetryInterval != null ? device.telemetryInterval + 's' : '5s')
        + '    </span>'
        + '    <button class="power-btn normal-btn" onclick="togglePowerMode(' + device.id + ', \'normal\')">NORMAL</button>'
        + '    <button class="power-btn low-btn" onclick="togglePowerMode(' + device.id + ', \'low\')">LOW POWER</button>'
        + '  </div>'
...
async function togglePowerMode(deviceId, mode) {
    try {
        await sendPowerModeCommand(deviceId, mode);
    } catch (error) {
        alert('Failed to send power mode command: ' + error.message);
    }
}
